"""
sql_schema.py - Emit schema.sql: CREATE TABLE DDL for all KSP-core + master +
extension tables. KSP-core/master DDL is byte-faithful to Police_FIR_ER_Diagram.pdf.
Extension tables are additive, clearly marked, and segregated.

Run:  python sql_schema.py
Output:  output/historical/db/schema.sql
"""
...
# ---------------------------------------------------------------------------
# KSP-CORE AND MASTER TABLE DDL  (ER-exact)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# KSP-CORE CASE TABLE DDL  (ER-exact)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# EXTENSION TABLE DDL  (additive; FKs to KSP core; context columns on link tables)
# ---------------------------------------------------------------------------
...
def emit_schema(output_path: str = None) -> str:
    if output_path is None:
        db_dir = Path("sample_data") / "historical" / "db"
    else:
        db_dir = Path(output_path).parent
    ...
    schema_path.write_text(full_ddl, encoding="utf-8")
    print(f"schema.sql written to {schema_path}")
